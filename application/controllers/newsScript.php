...
class ProduceScript
{
	public static function produce($option)
	{
        ...
        if($option['encoding'])
        {
            $script .= "{$tab}{$tab}\$options['encoding'] = '".$option['encoding']."';{$break}";
        }
		...
		$script .= "{$tab}{$tab}if(\$options['data']['page']>10){$break}{$tab}{$tab}{{$break}{$tab}{$tab}{$tab}return;{$break}{$tab}{$tab}}{$break}";
        ...
        if("" != $option["DetailPage"]["content"]){
            ...
            if($option["DetailPage"]["filter"])
            {
                ...
                $script .= "{$tab}{$tab}{$tab}{$tab}\$options['data']['content'] .= \$this->Filter(\$contentChildren[\$i]->outertext);{$break}";
                $script .= "{$tab}{$tab}{$tab}}{$break}";
                $script .= "{$tab}{$tab}}{$break}{$break}";
            }
            else
            {
                $script .= "{$tab}{$tab}for(\$i = 0; \$i < count(\$content); ++\$i){$break}";
                $script .= "{$tab}{$tab}{{$break}";
                $script .= "{$tab}{$tab}{$tab}\$options['data']['content'] .= trim(\$content[\$i]->outertext);{$break}";
                $script .= "{$tab}{$tab}}{$break}{$break}";
            }
		...
		$script .= "{$tab}{$tab}{$tab}if(empty(\$options['data']['title']) || empty(\$options['data']['content']) || empty(\$options['data']['pubtime']) || empty(\$options['data']['url'])){$break}";
        $script .= "{$tab}{$tab}{$tab}{{$break}";
        $script .= "{$tab}{$tab}{$tab}{$tab}\$this->SetError();{$break}";
        $script .= "{$tab}{$tab}{$tab}}{$break}";
		$script .= "{$tab}{$tab}{$tab}\$this->AddResult(\$options['data']);{$break}{$tab}{$tab}}{$break}{$tab}}{$break}{$break}";
			
        if($option["DetailPage"]["filter"])
        {
            $script .= "{$tab}public function Filter(\$text){$break}";
            $script .="{$tab}{{$break}";

            $tmp = "{$tab}{$tab}\$fileArr = array(";

            foreach($option["DetailPage"]["filter"] as $key=>$value )
            {
                if(trim($value))
                {
                    $tmp .= "'{$value}',";
            ...
            $script .="{$tab}{$tab}{{$break}";
            $script .="{$tab}{$tab}{$tab}\$text = str_replace(\$value,'',\$text);{$break}";
            $script .="{$tab}{$tab}}{$break}";
            $script .="{$tab}{$tab}return \$text;{$break}";
            $script .="{$tab}}{$break}";
        }
